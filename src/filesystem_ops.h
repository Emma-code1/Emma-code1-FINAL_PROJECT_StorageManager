// src/filesystem_ops.h
#ifndef FILESYSTEM_OPS_H
#define FILESYSTEM_OPS_H
typedef enum { FS_TYPE_EXT4, FS_TYPE_XFS, FS_TYPE_BTRFS } fs_type_t;
typedef struct {
    char device[256];
...
int fs_mount(const char *device, const char *mount_point, fs_type_t type, const char *options);
